...
function expectedMatchesPerTeam(groupSize: number): number {
  return Math.max(1, groupSize - 1);
}

function maxPoints(record: TeamRecord, expectedPlayed: number): number {
 ...
 */
export function isGroupMathematicallyEliminated(
  teamId: string,
  standings: GroupStanding[],
  ...
  if (rank === 4) {
    ...
    if (third && teamMax < third.points) return true;
    if (second && teamMax < second.points) return true;
    return false;
  }
  ...
  if (gap > remaining * 3) return true;

  return false;
}

/** Third-place teams that still have a mathematical path (group-level only). */
export function collectAliveThirdPlaceTeams(
  standings: GroupStanding[],
  context: QualificationMatchContext = { lockedGroupMatchCount: {}, lockedStandingsByGroup: {} }
): TeamRecord[] {
  const thirds = standings.map((s) => s.rows[2]).filter((r): r is TeamRecord => Boolean(r));
  return thirds.filter((r) => !isGroupMathematicallyEliminated(r.teamId, standings, context));
}

export function rankAliveBestThirds(
  standings: GroupStanding[],
  context: QualificationMatchContext = { lockedGroupMatchCount: {}, lockedStandingsByGroup: {} }
): TeamRecord[] {
...
export function isKnockoutEliminated(
  teamId: string,
  standings: GroupStanding[],
  context: QualificationMatchContext = { lockedGroupMatchCount: {}, lockedStandingsByGroup: {} }
): boolean {
  ...
  if (isGroupMathematicallyEliminated(teamId, standings, context)) return true;

  if (info.rank === 3) {
    const aliveRanked = rankAliveBestThirds(standings, context);
    const idx = aliveRanked.findIndex((r) => r.teamId === teamId);
    if (idx < 0) return true;
    if (info.groupComplete && idx >= 8) return true;
    return false;
  }
...
export function thirdPlaceRankAmongAlive(
  teamId: string,
  standings: GroupStanding[],
  context: QualificationMatchContext
): number {
  return rankThirdPlaceRecords(
    standings
      .map((s) => s.rows[2])
      .filter((r): r is TeamRecord => Boolean(r) && !isGroupMathematicallyEliminated(r.teamId, standings, context))
